...
<Controls:MetroWindow 
	...
	Title="Quick AD Support - User Analyze Part" 
	Width="470" 
	ResizeMode="NoResize"
	Height="Auto" 
					...
					<Expander Name="Expander_More_options" Header="Other options"  Margin="0,0,0,0" Background="gray" IsExpanded="False" Height="auto">  
						<Grid Background="white" HorizontalAlignment="Center">
							<StackPanel Orientation="Vertical" Margin="0,0,0,0">						
								<StackPanel Orientation="Horizontal">
								<!--
									<Button Width="40" ToolTip="Export User properties to a CSV file" Name="Export_Values" BorderThickness="0" Margin="0,0,0,0" 
										Style="{DynamicResource SquareButtonStyle}" Cursor="Hand" Background="#2196f3">
										<Rectangle Width="20" Height="20"  Fill="white" >
											<Rectangle.OpacityMask>
...
# $Export_Values = $Users_Window.findname("Export_Values") 
...
# $Expander_Basic_Block.Visibility = "Collapsed"
# $Expander_More_Block.Visibility = "Collapsed"
# $Expander_More_options_Block.Visibility = "Collapsed"
...
<Controls:MetroWindow 
	...
	xmlns:i="http://schemas.microsoft.com/expression/2010/interactivity"		
	xmlns:Controls="clr-namespace:MahApps.Metro.Controls;assembly=MahApps.Metro"
	Title="Quick AD Support - Computer Analyze Part" 
	Width="470" 
	ResizeMode="NoResize"
	Height="Auto" 
	SizeToContent="Height" 			
...
# Actions block
# $Expander_Change_Password_Block = $Computers_Window.findname("Expander_Change_Password_Block") 
# $Expander_Change_Password = $Computers_Window.findname("Expander_Change_Password") 
# $First_Password = $Computers_Window.findname("First_Password") 
# $Confirm_Password = $Computers_Window.findname("Confirm_Password") 
# $Set_Password = $Computers_Window.findname("Set_Password") 
...
$About_Version.Content = "Version: 1.0"
...
$Menu_Users.Text = "User Analyze"

$Menu_Computers = New-Object System.Windows.Forms.MenuItem
$Menu_Computers.Text = "Computer Analyze"
...
# ---------------------------------------------------------------------
# Action after clicking on User analyze
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Action after clicking on Computer analyze
# ---------------------------------------------------------------------
					...
					If($result -eq "Affirmative")
						{
							Try	
								{
									$pwd = convertto-securestring -string $First_PWD -asplaintext -force
...
$Computer_Enable_Account.Add_Click({
	...
	If (($Get_Computer_Infos.Enabled) -eq "True")	
		{
			$okAndCancel = [MahApps.Metro.Controls.Dialogs.MessageDialogStyle]::AffirmativeAndNegative
			$result = [MahApps.Metro.Controls.Dialogs.DialogManager]::ShowModalMessageExternal($Computer_Window,"Disable account","Do you want to disable the computer $computer ?",$okAndCancel)
			If($result -eq "Affirmative")
				{
					Try	
						{
					...
					Catch
						{
							[MahApps.Metro.Controls.Dialogs.DialogManager]::ShowMessageAsync($Computer_Window, "Oops :-(", "Can not disable the account. Check your rights !!!")																			
						}
				}
			Else
				{
	...
	Else
		{
			$okAndCancel = [MahApps.Metro.Controls.Dialogs.MessageDialogStyle]::AffirmativeAndNegative
			$result = [MahApps.Metro.Controls.Dialogs.DialogManager]::ShowModalMessageExternal($Computer_Window,"Enable account","Do you want to enable the computer $computer ?",$okAndCancel)
			If($result -eq "Affirmative")
				{
					Try	
						{
...
# $Delete_Computer_Account.Add_Click({
	# $Computer = $Get_User_Infos.Name
	
	# $okAndCancel = [MahApps.Metro.Controls.Dialogs.MessageDialogStyle]::AffirmativeAndNegative
	# $result = [MahApps.Metro.Controls.Dialogs.DialogManager]::ShowModalMessageExternal($Computer_Window,"Delete account","Do you want to delete the account $Computer ?",$okAndCancel)
	# If($result -eq "Affirmative")
		# {
			# Try	
				# {
					# Remove-ADComputer $Computer	-confirm:$false								
					# [MahApps.Metro.Controls.Dialogs.DialogManager]::ShowMessageAsync($Computer_Window, "Success :-)", "Account $Computer has been successfully deleted")									
				# }
			# Catch
				# {							
					# [MahApps.Metro.Controls.Dialogs.DialogManager]::ShowMessageAsync($Computer_Window, "Oops :-(", "Can not delete the account. Check your rights !!!")													
				# }						
		# }
	# Else
		# {
			...
			Try	
				{
					...
					Unlock-ADAccount -Identity $User
					System.Windows.Forms.MessageBox]::Show("User $User has been successfully unlocked")	
...
$Computer_Unlock_Account.Add_Click({
	...
	$okAndCancel = [MahApps.Metro.Controls.Dialogs.MessageDialogStyle]::AffirmativeAndNegative
	$result = [MahApps.Metro.Controls.Dialogs.DialogManager]::ShowModalMessageExternal($Computer_Window,"Unlock account","Do you want to unlock the account $computer ?",$okAndCancel)
	If($result -eq "Affirmative")
		{
			Try	
				{				
					Unlock-ADAccount -Identity $computer
					System.Windows.Forms.MessageBox]::Show("Computer $computer has been successfully unlocked")	
					[MahApps.Metro.Controls.Dialogs.DialogManager]::ShowMessageAsync($Computer_Window, "Oops :-(", "Computer $computer has been successfully unlocked")													
					
				}
			Catch
				{
					[MahApps.Metro.Controls.Dialogs.DialogManager]::ShowMessageAsync($Computer_Window, "Oops :-(", "Can not unlock the account $computer. Check your rights !!!")													
				}	
		}
	Else
		{
...
$Check_User_btn.Add_Click({
	$MyUser = $User_TxtBox.Text.ToString()
...
$Check_Computer_btn.Add_Click({
	...
	If ($MyComputer -ne "")
		{		
			...
			$Get_Computer_Infos = Get-ADComputer -Filter {(name -like $MyComputer)} -properties *	
...
$DatagGrid_Users.Add_MouseDoubleClick({
	...
	$Get_User_Infos = Get-ADUser -Filter {(SamAccountName -like $LogonAccount)} -properties *
	User_Found_Custo											
})
...
$DatagGrid_Computers.Add_MouseDoubleClick({
	...
	$Get_Computer_Infos = Get-ADComputer -Filter {(name -like $Computer)} -properties *	
	Computer_Found_Custo											
})
